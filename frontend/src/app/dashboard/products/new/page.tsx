...
export default function NewProductPage() {
  ...
  const [activeVisualValue, setActiveVisualValue] = useState<string>('');
  ...
  const generateVariants = (updatedOptions: typeof options) => {
    ...
    const newVariants = combinations.map((combo: any[]) => {
      const variantObj: any = { stock: '1', images: [] };
      updatedOptions.forEach((opt, i) => {
        if (opt.name.toLowerCase().includes('color')) variantObj.color = combo[i];
        else if (opt.name.toLowerCase().includes('size')) variantObj.size = combo[i];
        else variantObj[opt.name.toLowerCase()] = combo[i];
      });
  ...
  return (
    <div className="animate-fade-in position-relative">
        {toast.show && (
            <div className="position-fixed top-0 start-50 translate-middle-x mt-4 p-3" style={{ zIndex: 9999 }}>
                <div className={`shadow-lg rounded-pill px-4 py-3 d-flex align-items-center gap-3 animate__animated animate__fadeInDown ${toast.variant === 'success' ? 'bg-dark text-white' : 'bg-danger text-white'}`}>
                    <div className={`rounded-circle d-flex align-items-center justify-content-center bg-white ${toast.variant === 'success' ? 'text-dark' : 'text-danger'}`} style={{ width: 24, height: 24 }}>
                        <i className={`bi ${toast.variant === 'success' ? 'bi-check-lg' : 'bi-exclamation-lg'}`} style={{ fontSize: '16px' }}></i>
                    </div>
      ...
      <div className="mb-4">
        <Link href="/dashboard/products" className="text-dark text-decoration-none small fw-bold">
      ...
      <div className="row">
              ...
              <div className="row g-4">
                  ...
                  <div className="col-12 animate-fade-in">
                    <div className="row g-4">
                      <div className="col-12">
                        ...
                        <input type="text" name="name" className="form-control form-control-lg rounded-3" placeholder="e.g., Premium Cotton T-Shirt" value={formData.name} onChange={handleChange} required />
                      </div>
                      ...
                      <div className="col-12">
                        ...
                        <textarea name="description" className="form-control rounded-3" rows={6} placeholder="Describe your product in detail..." value={formData.description} onChange={handleChange} />
                      </div>
                      <div className="col-12 text-end pt-3">
                         <button type="button" className="btn btn-dark px-4 fw-bold rounded-pill" onClick={() => setActiveTab('options')}>Continue to Options</button>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {activeTab === 'options' && (
                  <div className="col-12 animate-fade-in">
                    <div className="variation-theme-box shadow-sm mb-4">
                        ...
                        <p className="small text-muted mb-4">Define product variants (e.g. Size, Color).</p>
                           ...
                           <div key={optIdx} className="mb-4 pb-3 border-bottom">
                              <div className="row g-2 align-items-center mb-2">
                                 ...
                                 <div className="col-md-8">
                                    ...
                                    <input type="text" placeholder="Type and press Enter" className="form-control form-control-sm" onKeyDown={(e: any) => {
                                        if (e.key === 'Enter') { e.preventDefault(); addOptionValue(optIdx, e.target.value); e.target.value = ''; }
                                    }} />
                                 </div>
                                 <div className="col-md-1 text-end pt-4">
                                    <button type="button" onClick={() => removeOption(optIdx)} className="btn btn-link text-danger p-0"><i className="bi bi-trash"></i></button>
                                 </div>
                              </div>
                        ...
                        {options.length < 3 && (
                          <button type="button" onClick={addOption} className="btn btn-outline-dark btn-sm rounded-pill fw-bold">
                            <i className="bi bi-plus-lg me-1"></i> Add Another Property
                          </button>
                        )}
                        {options.length === 0 && <p className="small text-muted mb-0">No options? This will be a single listing.</p>}
                    </div>
                    <div className="col-12 text-end mt-5 border-top pt-4">
                       <button type="button" className="btn btn-dark px-4 fw-bold rounded-pill" onClick={() => setActiveTab('inventory')}>Continue to Inventory</button>
                    </div>
                  </div>
                )}
                ...
                {activeTab === 'inventory' && (
                  <div className="col-12 animate-fade-in">
                    {variants.length > 0 ? (
                      <div className="mt-2">
                         <div className="row g-4 pt-2">
                            <div className="col-12">
                                <div className="alert alert-info">Set stock levels for each combination.</div>
                            </div>
                         </div>
                         <div className="table-responsive">
                            <table className="amazon-variant-table shadow-sm">
                               <thead><tr><th>Specific Item</th><th>Stock</th></tr></thead>
                               <tbody>
                                  {variants.map((v, i) => (
                                      <tr key={i}>
                                          ...
                                          <td>
                                              <div className="input-group input-group-sm">
                                                <input type="number" className="form-control border-dark border-2 fw-bold text-center" style={{maxWidth: '80px'}} value={v.stock} onChange={(e) => handleVariantStockChange(i, e.target.value)} />
                                                <button type="button" className="btn btn-outline-dark px-1 x-small fw-bold" onClick={() => handleVariantStockChange(i, ((parseInt(v.stock) || 0) + 5).toString())}>+5</button>
                                                <button type="button" className="btn btn-outline-dark px-1 x-small fw-bold" onClick={() => handleVariantStockChange(i, ((parseInt(v.stock) || 0) + 10).toString())}>+10</button>
                                              </div>
                                          </td>
                                      </tr>
                                  ))}
                      ...
                      <div className="row g-4 pt-2">
                         <div className="col-md-6 text-center mx-auto py-5">
                            <label className="fw-bold small text-uppercase text-muted mb-3 d-block">Physical Stock Quantity</label>
                    ...
                    <div className="col-12 text-end mt-5 border-top pt-4">
                       <button type="button" className="btn btn-dark px-4 fw-bold rounded-pill" onClick={() => setActiveTab('images')}>Continue to Pricing & Photos</button>
                    </div>
                  </div>
                )}
                      ...
                      <div className="row g-4">
                                            ...
                                            <button 
                                                ...
                                                onClick={() => {
                                                    ...
                                                    if (imgToUse) {
                                                        ...
                                                        // New page doesn't have isDirty check usually, but state updates work
                                                        alert('Thumbnail updated from active variant!');
                                                    } else {
                                                        alert('No variant images found to copy.');
                                                    }
                                                }}
                                            >
                                                ...
                                                <div className="bg-white p-2 rounded-3 border">
                                                    <div className="input-group input-group-sm">
                                                        ...
                                                        <button className="btn btn-dark rounded-pill px-3 fw-bold" type="button" onClick={() => {
                                                            const inp = document.getElementById('gallery-input-new') as HTMLInputElement;
                                                            if (inp && inp.value) {
                        ...
                        <div className="col-12 mt-3 pt-3 border-top text-end sticky-bottom bg-white p-3 shadow-sm rounded-4" style={{zIndex: 100, bottom: 20}}>
                          <div className="d-flex align-items-center justify-content-end gap-3">
                              <span className="small text-muted fw-bold">Ready to publish?</span>
                              <button type="submit" className="btn btn-dark btn-lg px-5 fw-bold rounded-pill shadow-sm hover-scale" disabled={loading}>
        ...
        <div className="col-lg-3">
          <div className="dashboard-card bg-light border-0 shadow-sm sticky-top" style={{ top: '24px' }}>
            ...
            <h6 className="fw-bold small mb-1">{formData.name || 'Product Title'}</h6>
            <p className="h5 fw-bold text-success mb-2">{formData.price ? formData.price + ' EGP' : '0 EGP'}</p>
          </div>
        </div>
